...
首先，onSaveInstanceState的调用时机，它是在当前Activity可能会被杀死的时候才会触发，什么场景下会被杀死呢？当前Activity不可见，不可见的时候，就存在被回收的可能，如果用户主动的去杀死当前Activity，这个方法是不会被调用的。
...
这个问题，我想stackoverflow上有非常多的解决方案，但是从根本上去说，是因为onCreate的时候，元素还不可见，所以做不了动画。


> onPause中处理时间过长，是否会引起ANR呢？

会，官方要求操作在500ms内处理完毕，超过了5s，会引发ANR。
